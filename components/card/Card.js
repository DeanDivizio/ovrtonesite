...
const Card = ({ title, description, image, link, bgImage, smallTXT, background, goToContact }) => {
    ...
    return (
        <div className="flex flex-col justify-center items-center h-full px-8 lg:px-24 py-16 md:py-28 max-w-[450px] min-w-[200px] lg:min-w-[300px] min-h-96 rounded-2xl bg-cover shadow-[0_0px_36px_-8px_rgba(255,255,255,0.05)]" style={cardStyle}>
            ...
            {goToContact && (<button className="bg-primary-dim hover:bg-primary-light transition px-8 pt-5 pb-3 rounded-xl text-white font-medium text-xl lg:text-2xl tracking-wide" onClick={handleContactClick}>Get Started</button>)}
        </div>
    );
 };
export default Card;