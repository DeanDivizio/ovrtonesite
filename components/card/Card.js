...
const Card = ({ title, description, image, link, bgImage, smallTXT, background, goToContact }) => {
    ...
    return (
        <div className="flex flex-col justify-center items-center h-full px-16 md:px-24 py-16 md:py-28 max-w-[450px] min-w-[350px] min-h-96 rounded-2xl bg-cover shadow-[0_0px_36px_-8px_rgba(255,255,255,0.05)]" style={cardStyle}>
            {link || goToContact ? <h3 className="mb-0 text-center leading-snug">{title}</h3> : <h3 className="mb-6 text-center leading-snug">{title}</h3> }
            {description ? (description && smallTXT ? (<p className="text-center mb-8" style={{fontSize:'1.5rem'}}>{description}</p>) : (<p className="text-center mb-8">{description}</p>)) : <hr className="w-[30%] border border-off-white mb-8"></hr> }
